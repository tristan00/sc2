...
def train_strat_model():
    ...
    # model = models.Sequential()
    # model.add(layers.LSTM(units=128, input_shape=(memory_size, var_size,), return_sequences=True))
    # model.add(layers.LSTM(units=128))
    # model.add(layers.Dense(pos_y.shape[0], activation='sigmoid'))
    # model.compile(loss='categorical_crossentropy', optimizer='adam',
    #               metrics=['accuracy'])
    #
    # x_train = x_train.reshape(memory_size, -1, var_size)
...
class Strat():
    ...
    def get_move(self, move_dict, game_state):
        if random.random() < self.random_chance or not self.use_model:
            ...
            scaled_input = self.scaler.transform(np.array([game_state]))
...
class SentdeBot(sc2.BotAI):
    ...
    def get_state(self):
        ...
        kvu = self.state.score.killed_value_units
        ...
        return [dead_unit_count, unit_count, score, game_loop, minerals, supply_cap, supply_left, num_th, keu, kes,
                vg, l_vg, n_probes, n_gates, n_cyber, n_stalker,n_zealot,n_stargate,n_void,n_robo,n_im,kvu,
